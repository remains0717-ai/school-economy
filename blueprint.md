...
6.  **전문가급 주식 투자 시스템:**
    ...
    *   실제 증권사 앱 스타일의 매수/매도 UI 및 탭 시스템 구현. (완료)
    *   환율 시뮬레이션을 포함한 정확한 주문 금액 계산 및 잔액 검증 로직 적용. (완료)
    *   종목 검색 기능 고도화 (NASDAQ, BINANCE 등 주요 거래소 연동). (완료)
7.  `index.html`: Firebase SDK 로드 및 인증 UI 요소 추가. (완료)
